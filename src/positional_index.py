...
# Positional index to support phrase queries.
...
def phrase_query(query_tokens, pos_index):
    """
    Return set of doc_ids where query_tokens appear as a contiguous phrase.
    Simple algorithm: check pairwise positional offsets.
    """
    if not query_tokens:
        return set()
    ...
    # start with docs that contain first token
    ...
    for doc in candidate_docs:
        ...
        # check if there is any position p in positions_list[0] such that
        # p+1 in positions_list[1], p+2 in positions_list[2], ...
